...
class FrozenLakeRewardWrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        self._rows, self._cols = desc.shape # returns 8x8 for 8x8 map
        self._goal_rc = tuple(map(int, np.argwhere(desc == b'G')[0])) # goal state for map ((7,7))
        return obs, info
    ...
    def step(self, action):
        ...
        if prev_obs is None:
            ...
            if len(self._visited) > 1:
                # Not perfect, but avoids adding state to info in the base env.
                prev_obs = obs
